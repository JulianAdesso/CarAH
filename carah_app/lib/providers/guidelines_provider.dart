...
import '../model/content_image.dart';
...
class GuidelinesProvider extends ContentProvider<Guideline> {
  ...
  GuidelinesProvider({required this.settingsProvider}) {
    settingsProvider.getSettingsOfUser();
  }

  GuidelinesProvider update(SettingsProvider settingsProvider) {
  ...
  fetchDataByCategory(String uuid) async {
    ...
    if (connectivityResult == ConnectivityResult.mobile ||
      ...
      var guidelineFromCMS = await http.post(Uri.parse('$baseUrl/graphql'),
          headers: {
    ...
    if (!settingsProvider.userSettings!.dataSaveMode) {
      ...
      if ((connectivityResult == ConnectivityResult.mobile ||
        ...
        var message = await http.get(
            Uri.parse('$baseUrl/nodes/${image.uuid}/binary/image'),
            headers: {
              "Content-Type": "application/json",
            });
  ...
  void replaceContentPlaceholdersWithImages(Guideline guideline) {
    ...
    guideline.content =
        guideline.content.replaceAllMapped(imgIdentifier, (match) {
      String imgTag = '';
      ...
      if (guideline.images != null && imgName != null) {
        ...
        Iterable<ContentImage> allFittingImages = guideline.images!
