...
class CategoryProvider extends ChangeNotifier {
  ...
  Future<Category> fetchCategoryById(String categoryUUID, String type) async {
    ...
    if (connectivityResult == ConnectivityResult.mobile ||
      ...
      List<Category> offlineCategories =
          _offlineBox.get(type)?.cast<Category>();
      category = offlineCategories
          .where((element) => element.uuid == categoryUUID)
          .toList()
          .first;
    }
    notifyListeners();
  ...
  Future<bool> downloadCategory(String categoryID) async {
    List<Category>? cat =
        _offlineBox.get('articles_category')?.cast<Category>();
    if (cat == null || !cat.any((element) => element.uuid == categoryID)) {
      Category searchedCategory =
          await fetchCategoryById(categoryID, 'articles_category');
      cat != null ? cat.add(searchedCategory) : cat = [searchedCategory];
    }
  ...
  Future<bool> removeDownloadFromCategories(String categoryID) async {
    List<Category>? cat =
        await _offlineBox.get('articles_category')?.cast<Category>();
